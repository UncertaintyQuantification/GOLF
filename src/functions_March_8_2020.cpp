...
//const List& R0_1
//const VectorXi&  d_vec
// [[Rcpp::export]] 
List GOLF_3D_MCMC(const MatrixXd& Y_sample, const List& R0_1,
                  const VectorXd&  delta_x_2, String kernel_type_spatial,String kernel_type_temporal,
                  const int M, const int M_0, const VectorXi& d_vec, const VectorXi& missing_index, const MatrixXi& missing_index_all_mat,
                  const VectorXd& prior_par,const VectorXd& initial_kernel_par,
                  const VectorXd& step_size_kernel_par,const int seed_here,
                  const bool need_interval,const double interval_prop, const bool have_mean, const MatrixXd& H2){
  ...
  int d1=d_vec[0];
  int d2=d_vec[1];
  int d=d_vec[0]*d_vec[1];
  
  
  VectorXd log_beta_eta_cur=initial_kernel_par.head(2*d);
  double log_beta11_cur=initial_kernel_par[2*d];
  double log_beta12_cur=initial_kernel_par[2*d+1];
  
  MatrixXd R0_11=R0_1[0];
  MatrixXd R0_12=R0_1[1];
  
  int n11= R0_11.cols();
  int n12= R0_12.cols();
  
  List eigen_R11_cur=Get_eigs_1(R0_11,log_beta11_cur,kernel_type_spatial,d1);
  List eigen_R12_cur=Get_eigs_1(R0_12,log_beta12_cur,kernel_type_spatial,d2);
  
  MatrixXd eigen_R1_vectors_cur;
  
  VectorXd eigen_R11_values_cur=eigen_R11_cur[0];
  MatrixXd eigen_R11_vectors_cur=eigen_R11_cur[1];
  
  VectorXd eigen_R12_values_cur=eigen_R12_cur[0];
  MatrixXd eigen_R12_vectors_cur=eigen_R12_cur[1];
  
//kronecker(eigen_R12_cur$values,eigen_R11_cur$values,'*')
    
  MatrixXd  eigen_R1_values_cur=(eigen_R12_values_cur*eigen_R11_values_cur.transpose()).transpose();
  eigen_R1_values_cur.resize(d,1);
  ...
  for(int i_d=0;i_d<d;i_d++){
    ...
    Cov_here=Construct_Cor_KF_no_C_R_K_Q(log_beta_eta_cur[i_d],delta_x_2,index_obs,kernel_type_temporal);
  ...
  //getting Y_sample_tilde_cur
  MatrixXd Y_sample_mean_sub_resize=Y_sample_here-mean_cur;
  Y_sample_mean_sub_resize.resize(n11,n12*n2);
  MatrixXd part1=eigen_R11_vectors_cur.transpose()*Y_sample_mean_sub_resize;
  MatrixXd part1_transpose_resize=part1.transpose();
  part1_transpose_resize.resize(n12,d1*n2);
  MatrixXd part2=eigen_R12_vectors_cur.transpose()*part1_transpose_resize;
  MatrixXd Y_sample_tilde_cur_transpose=part2.transpose();
  Y_sample_tilde_cur_transpose.resize(n2,d1*d2);
  MatrixXd Y_sample_tilde_cur=Y_sample_tilde_cur_transpose.transpose();
    
  //  t(matrix(t(part2),n2,d1*d2))
  ...
  for(int i_d=0;i_d<d;i_d++){
    ...
    //note here the prior par we only need the last four. The following function is written for 2d
    beta_post_cur.row(i_d)=(Get_beta_lik_S_2_log_det(Y_sample_tilde_cur.row(i_d).transpose(),log_beta_eta,tilde_eta_vec_cur[i_d],Cov_2_cur[i_d],C_R_K_Q_cur[i_d],add_log_prior,prior_par.tail(4),n2,sigma_2_0_cur)).transpose();
  ...
  MatrixXd param_record=MatrixXd::Zero(M,2*d+3);
  ...
  double log_JR_prior1_cur;
  double log_post_beta1_cur;
  VectorXd CL11=Eigen::VectorXd::Zero(1);
  VectorXd CL12=Eigen::VectorXd::Zero(1);
  
  CL11[0]=prior_par[0];
  CL12[0]=prior_par[1];
  double log_beta11_propose;
  double log_beta12_propose;
  List eigen_R11_propose;
  List eigen_R12_propose;
  
  MatrixXd eigen_R1_values_propose;
  VectorXd eigen_R11_values_propose;
  VectorXd eigen_R12_values_propose;
  
  MatrixXd eigen_R11_vectors_propose;
  MatrixXd eigen_R12_vectors_propose;
  
  List C_R_K_Q_propose(d);
  List C_R_K_Q_beta1_propose(d);
  VectorXd tilde_eta_vec_propose;
  VectorXd log_beta11_cur_vec=VectorXd::Zero(1);
  VectorXd log_beta12_cur_vec=VectorXd::Zero(1);
  
  MatrixXd Y_sample_tilde_propose; 
  MatrixXd Y_sample_tilde_propose_transpose;
  double Trace_Y_sample_tilde_2_propose;
  List C_R_K_Q_here;
  MatrixXd beta_post_propose=MatrixXd::Zero(d,3);
  VectorXd Q_here;
  ...
  VectorXd CL2=Eigen::VectorXd::Zero(1);
  CL2[0]=prior_par[2];
  VectorXd log_beta11_propose_vec=Eigen::VectorXd::Zero(1);
  VectorXd log_beta12_propose_vec=Eigen::VectorXd::Zero(1);
  ...
  for(int i_M=0; i_M<M;i_M++){
    ...
    for(int i_row=0;i_row<d;i_row++){
      ...
      tilde_eta_propose=exp(log_eta_propose)/eigen_R1_values_cur(i_row,0);
      Cov_2_propose=Construct_Cor_KF_no_C_R_K_Q(log_beta_propose,delta_x_2,index_obs,kernel_type_temporal);
      ...
      beta_post_S_2_log_det_propose=(Get_beta_lik_S_2_log_det(Y_sample_tilde_cur.row(i_row).transpose(),log_beta_eta_propose,tilde_eta_propose,Cov_2_propose,C_R_K_Q_propose[i_row],add_log_prior,prior_par.tail(4),n2,sigma_2_0_cur)).transpose();
    ...
    log_beta11_cur_vec(0)=log_beta11_cur;
    log_beta12_cur_vec(0)=log_beta12_cur;
    
    log_JR_prior1_cur=log_approx_ref_prior(log_beta11_cur_vec,0,false,CL11,prior_par[3],prior_par[4])+log_approx_ref_prior(log_beta12_cur_vec,0,false,CL12,prior_par[3],prior_par[4]);
    
    log_post_beta1_cur=n2/2.0*tilde_eta_vec_cur.array().log().sum()-1/2.0*beta_post_cur.col(2).sum()-(Trace_Y_sample_2-Trace_Y_sample_tilde_2_cur+beta_post_cur.col(1).array().sum())/(2.0*sigma_2_0_cur)+log_JR_prior1_cur+log_beta11_cur+log_beta12_cur;
    
    //new
    //log_beta11_propose=log_beta11_cur+step_size_kernel_par[2]*R::rnorm(0,1.0); 
    //log_beta12_propose=log_beta12_cur+step_size_kernel_par[3]*R::rnorm(0,1.0); 
    
    log_beta11_propose=log_beta11_cur+step_size_kernel_par[2]*(distribution_norm(generator)); 
    log_beta12_propose=log_beta12_cur+step_size_kernel_par[3]*(distribution_norm(generator)); 
    
    
    eigen_R11_propose=Get_eigs_1(R0_11,log_beta11_propose,kernel_type_spatial,d1);
    eigen_R11_values_propose=eigen_R11_propose[0];
    eigen_R11_vectors_propose=eigen_R11_propose[1];
    
    eigen_R12_propose=Get_eigs_1(R0_12,log_beta12_propose,kernel_type_spatial,d2);
    eigen_R12_values_propose=eigen_R12_propose[0];
    eigen_R12_vectors_propose=eigen_R12_propose[1];
    
    eigen_R1_values_propose=(eigen_R12_values_propose*eigen_R11_values_propose.transpose()).transpose();
    eigen_R1_values_propose.resize(d,1);

    
    
    tilde_eta_vec_propose=(log_beta_eta_cur.segment(d,d).array().exp()/eigen_R1_values_propose.array()).matrix();
    
    Y_sample_mean_sub_resize=Y_sample_here-mean_cur;
    Y_sample_mean_sub_resize.resize(n11,n12*n2);
    part1=eigen_R11_vectors_propose.transpose()*Y_sample_mean_sub_resize;
    part1_transpose_resize=part1.transpose();
    part1_transpose_resize.resize(n12,d1*n2);
    part2=eigen_R12_vectors_propose.transpose()*part1_transpose_resize;
    Y_sample_tilde_propose_transpose=part2.transpose();
    Y_sample_tilde_propose_transpose.resize(n2,d1*d2);
    
    Y_sample_tilde_propose=Y_sample_tilde_propose_transpose.transpose();
    ...
    for(int i_row=0;i_row<d;i_row++){
      ...
      log_JR_prior=log_approx_ref_prior(log_beta_eta,0,true,CL2,prior_par[3],prior_par[4]);
    ...
    log_beta11_propose_vec(0)=log_beta11_propose;
    log_beta12_propose_vec(0)=log_beta12_propose;
    
    log_JR_prior1_propose=log_approx_ref_prior(log_beta11_propose_vec,0,false,CL11,prior_par[3],prior_par[4])+log_approx_ref_prior(log_beta12_propose_vec,0,false,CL12,prior_par[3],prior_par[4]);
    
    
    log_post_beta1_propose=n2/2.0*tilde_eta_vec_propose.array().log().sum()-1.0/2.0*beta_post_propose.col(2).sum()-(Trace_Y_sample_2-Trace_Y_sample_tilde_2_propose+beta_post_propose.col(1).array().sum())/(2.0*sigma_2_0_cur)+log_JR_prior1_propose+log_beta11_propose+log_beta12_propose;
    ...
    if(decision){
      log_beta11_cur=log_beta11_propose;
      log_beta12_cur=log_beta12_propose;
      
      C_R_K_Q_cur=clone(C_R_K_Q_beta1_propose); //this needs to be clone
      eigen_R11_cur=eigen_R11_propose;
      eigen_R12_cur=eigen_R12_propose;
      
      eigen_R1_values_cur=eigen_R1_values_propose;
      eigen_R11_vectors_cur=eigen_R11_vectors_propose;
      eigen_R12_vectors_cur=eigen_R12_vectors_propose;
    ...
    param_record(i_M,2*d)=exp(log_beta11_cur);
    param_record(i_M,2*d+1)=exp(log_beta12_cur);
    ...
    sigma_2_0_cur=1.0/tau_sample;
    param_record(i_M,2*d+2)=sigma_2_0_cur;
    ...
    if(have_mean){
      //A_T_Y_sample_cur=eigen_R1_vectors_cur.transpose()*(Y_sample_here);
      //getting Y_sample_tilde_cur
      Y_sample_mean_sub_resize=Y_sample_here;
      Y_sample_mean_sub_resize.resize(n11,n12*n2);
      part1=eigen_R11_vectors_cur.transpose()*Y_sample_mean_sub_resize;
      part1_transpose_resize=part1.transpose();
      part1_transpose_resize.resize(n12,d1*n2);
      part2=eigen_R12_vectors_cur.transpose()*part1_transpose_resize;
      Y_sample_tilde_cur_transpose=part2.transpose();
      Y_sample_tilde_cur_transpose.resize(n2,d1*d2);
      A_T_Y_sample_cur=Y_sample_tilde_cur_transpose.transpose();
      ...
      for(int i_row=0;i_row<d;i_row++){
        ...
        L_B2_tilde_sample = lltOfH2.matrixL();
        tilde_B2_hat.col(i_row)=H2_t_R_inv_H2_inv*H2_t_R_inv_y;
        tilde_B2_sample.col(i_row)= tilde_B2_hat.col(i_row)+sqrt(sigma_2_cur[i_row]*eigen_R1_values_cur(i_row,0))*(L_B2_tilde_sample*normal_sample_q2);
      ...
      eigen_R1_vectors_cur=kroneckerProduct(eigen_R12_vectors_cur,eigen_R11_vectors_cur).eval();
      ...
      Y_sample_tilde_cur_transpose=part2.transpose();
      Y_sample_tilde_cur_transpose.resize(n2,d1*d2);
      Y_sample_tilde_cur=Y_sample_tilde_cur_transpose.transpose();
    ...
    for(int i_row=0;i_row<d;i_row++){     
      ...
      if(kernel_type_temporal=="exp"){
        Cov_here=Cov_2_cur[i_row];
        KF_smoother_sample_list=Kalman_smoother_mean_sample_known_C_R_K_Q_exp(C_R_K_Q_cur[i_row],Cov_here[1],  Cov_here[0],  Cov_here[2],tilde_eta_vec_cur[i_row],
                                                                              index_obs,Y_sample_tilde_cur.row(i_row).transpose()/sqrt(eigen_R1_values_cur(i_row,0)*sigma_2_cur[i_row]),need_sample,need_mean,need_var);
    ...
    for(int i_missing=0;i_missing<n_missing; i_missing++){
      //Y_sample_missing[i_missing]=Y_sample_mean_all(missing_index_all_mat(i_missing,0),missing_index_all_mat(i_missing,1))+sigma_2_0_cur_sqrt*R::rnorm(0,1.0);
    ...
    for(int i_row=0;i_row<d;i_row++){
      ...
      beta_post_cur.row(i_row)=(Get_beta_lik_S_2_log_det(Y_sample_tilde_cur.row(i_row).transpose(),log_beta_eta,tilde_eta_vec_cur[i_row],Cov_2_cur[i_row],C_R_K_Q_cur[i_row],add_log_prior,prior_par.tail(4),n2,sigma_2_0_cur)).transpose();
  ...
  List return_List(5);
  //return_List[0]=Y_sample_here;
  /*
  return_List[0]=Y_sample_tilde_cur;
  return_List[1]=z_sample_matrix;
  return_List[2]=mean_cur;
  return_List[3]=Y_sample_mean_all;
  return_List[4]=Y_sample_missing;
*/
